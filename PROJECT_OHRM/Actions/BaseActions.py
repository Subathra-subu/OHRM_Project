...
class BaseActions:
    ...
    def js_click(self, locator):
        try:
            # Do NOT wait for element_to_be_clickable().
            # OrangeHRM search/filter buttons can temporarily fail
            # the clickable condition while the form is updating.
            element = self.wait.until(
                EC.visibility_of_element_located(locator)
            )
    ...
    def scroll_into_view(self, locator):
        try:
            ...
            self.logger.error(
                "Failed to scroll into view of the element"
            )
            self.logger.exception(e)
    ...
    def wait_for_url_contains(self, expected_partial_url):
        try:
            self.logger.info(
                f"Waiting dynamically for URL to contain: "
                f"'{expected_partial_url}'"
            )
            ...
            self.logger.error(
                f"URL failed to transition to target containing: "
                f"'{expected_partial_url}'"
            )
            self.logger.exception(e)
            raise
    ...
    def select_first_dropdown_option_via_js(self, listbox_locator):
        dropdown_element = self.wait_for_visibility(
            listbox_locator
        )
        ...
        WebDriverWait(self.driver, 7).until(
            lambda d:
            "searching" not in
            d.find_element(*listbox_locator).text.lower()
        )

        self.logger.info(
            "Dispatching custom browser lifecycle events "
            "onto the first dropdown option element..."
        )
        ...
        self.driver.execute_script(
            ...
            if (dropdown && dropdown.firstElementChild) {
                ...
                console.error(
                    "DOM Error: target listbox wrapper "
                    + "or option elements are absent."
                );
            }
            """,
            dropdown_element
    ...
    def wait_for_element_value_attribute(
        self,
        locator,
        timeout=10
    ):
    ...
    def save_screenshot(self, screenshot_name):
        try:
            ...
            self.logger.info(
                f"Screenshot saved successfully: {file_path}"
            )
    ...
    def clear_and_enter_text(self, locator, value):
        try:
            element = self.wait.until(
                EC.visibility_of_element_located(locator)
            )
